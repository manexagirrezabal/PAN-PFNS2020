...
if LANG == "en":
    ...
    from spellcheckeren import get_error_vector,initialize_vector
elif LANG == "es":
    from embeddings.embeddingsesp import wembs as wembsesp
    from spellcheckeres import get_error_vector,initialize_vector
else:
    print ("ERROR EMB")
    exit()
...
#    print (texts)
    ...
    user_error_vector = initialize_vector()
    for text in texts:
        error_vector = get_error_vector(text)
        user_error_vector = user_error_vector + error_vector

    return np.concatenate(([np.mean(lengths), np.std(lengths),upper_ratio],sumembs,devembs,bag_of_pos,user_error_vector))
...
fout = open("resultswithspellerrors"+LANG+".txt","w")
